...
2. Proper Caching and Indexing:-
   ...
   or Outbox pattern which will not slow the save query down.
   Proper indexing like creating a composite index like rating and cycleId will improve the performance rather well
   ...
   redis to cache db responses, as we are using multiple queries
...
3. Increasing pool size, introducing read replicas:-
   ...
   reports.
   This offloads read traffic from the primary database and increase database connection pool size so each instance can
   handle
...
1. Introducing Composite Indexing:
   ...
   for the cycle,
   then filter and sort in memory. As row count grows, this gets worse. Fix it with a composite index:
...
3. Maintaining a Separate Analytical Summary Tables:
   this will help to fetch values like average and counts quicker than doing on calculating at the instance.
